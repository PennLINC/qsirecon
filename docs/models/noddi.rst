...
and the orientation dispersion index (ODI) for neurite orientation variability :footcite:p:`noddi`.
These metrics disentangle factors contributing to diffusion anisotropy that were conflated in :ref:`DTI <dti_model>` measures like FA :footcite:p:`noddi`.
...
AMICO achieves a 1000× speedup in fitting time with minimal loss of accuracy :footcite:p:`amico`.
...
In 2021, :footcite:t:`parker2021not` introduced tissue-fraction-modulated ICVF and ODI,
scaling NODDI metrics by the tissue signal fraction (1 - ISOVF).
This adjustment was shown to remove artifactual group differences that were driven by differing CSF contamination rather than true tissue changes :footcite:p:`parker2021not`.
...
For example,
neurite density (ICVF) in major tracts increases steeply from birth to age 3 :footcite:p:`Jelescu2015WMTINODDI`,
...
Multiple studies (e.g. :footcite:t:`Genc2017NDIAgeDevelopment`, :footcite:t:`Mah2017NODDIChildDevelopment`)
...
indicating that fiber organization (coherence) does not markedly increase after early childhood in most regions :footcite:p:`Mah2017NODDIChildDevelopment`.
...
meaning microstructural density is a key determinant of developmental differences :footcite:p:`Mah2017NODDIChildDevelopment`.
...
:footcite:t:`Chang2015NODDIDataset` noted that NDI followed a logarithmic growth curve that leveled off in the 20s-30s,
...
and higher ODI across virtually all white matter tracts :footcite:p:`Lawrence2021UKBiobankWhiteMatter`.
...
:footcite:t:`Lawrence2021UKBiobankWhiteMatter` also found that CSF fraction (ISOVF)
...
Importantly, NDI has been found to be a better predictor of chronological age than standard DTI measures in youth :footcite:p:`Genc2017NDIAgeDevelopment`.
...
As a result, NODDI parameter estimates may be biased if these conditions aren't met :footcite:p:`guerrero2019optimizing`.
For example,
using the default d‖ in cortical gray matter can lead to overestimation of ICVF (since true diffusion is slower).
Investigators have to adjust this value for different tissues or accept some bias :footcite:p:`guerrero2019optimizing`.
...
This can lead to degenerate solutions where the fitting algorithm might converge on one of several "equivalent" parameter sets :footcite:p:`jelescu2017design`.
