...
def check_generated_files(qsirecon_dir, output_list_file, optional_output_list_file):
    """Compare files generated by qsirecon with a list of expected files."""
    found_files = sorted(glob(os.path.join(qsirecon_dir, "**/*"), recursive=True))
    found_files = [os.path.relpath(f, qsirecon_dir) for f in found_files]
